...
if ($_POST) {
	$conexion = new Mysql();
	$usuarioOne = new Usuario($_POST["usuario"], $_POST["contrasena"], $_POST["email"], $_POST["nombre"], $_POST["apellido"]);
	$data = $conexion->validarExisteUsuarioEmail($usuarioOne);
	if (!$data) {
		$ruta = subirAvatar($_FILES["avatar"]);
		$usuarioOne->setRutaAvatar($ruta);
		$conexion->guardarUsuario($usuarioOne);
		header('Location: login.php');
	} else {
		
		// Mostrar Mensaje de que el usuario o email ya existe...
		var_dump("TODO mostrat mensaje de emial/usuario existente!");
	}
}
